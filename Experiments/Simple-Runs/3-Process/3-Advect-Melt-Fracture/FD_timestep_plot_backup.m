...
if mod(FSTD.i,1) == 0
    ...
    if FSTD.i == 1
        ...
        if FSTD.i == OPTS.nt
            ...
            imcontour([0 FSTD.time/86400],FSTD.Rint,log10(squeeze(sum(bsxfun(@times,DIAG.FSTD.psi,DIAG.FSTD.dA),2)+eps)),[-4:.1:0],'--k','showtext','on');
            set(gca,'clim',[-4 0])
    ...
    if length(FSTD.Hmid) > 1
        if FSTD.i == 1
            ...
            if FSTD.i == OPTS.nt
                ...
                imcontour([0 FSTD.time/86400],FSTD.Hmid,log10(squeeze(sum(bsxfun(@times,DIAG.FSTD.psi,DIAG.FSTD.dA),1)+eps)),[-4:.1:0],'--k','showtext','on');
                set(gca,'clim',[-4 0])
