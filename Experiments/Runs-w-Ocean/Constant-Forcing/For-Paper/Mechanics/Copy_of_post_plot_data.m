...
plot(abciss,horvat_alpha(2:d),'--','color',cplots(1,:))
...
for i = 1:length(coeff)
    ...
    yapprox =  polyval(-[horvat_alpha(i),0],log10(Rinert));
    % yapprox = 10.^(yapprox); 
    yapprox = yapprox / sum(yapprox); 
    yleg = FSDinertlog(:,i)' / sum(FSDinertlog(:,i)'); 
        
    err_pl(i) = norm((yapprox - yleg)); 
    
    yapprox = polyval(-[coeff(i) coeff2(i)],log10(Rinert));
    % yapprox = 10.^(yapprox); 
    % yapprox = sum(FSDinertlog(:,i)) * yapprox / sum(yapprox); 
    yapprox = yapprox / sum(yapprox); 

    
    err_fit(i) = norm((yapprox - yleg)); % / (norm(FSDinertlog(:,i))); 
    
end


plot(abciss,err_pl(1:end-1),'-','color',cplots(1,:))

hold on

plot(abciss,err_fit(1:end-1),'-','color',cplots(2,:)); 

legend('Error in P+J','Error in Fitting','location','south')
...
saveas(gcf,'~/Dropbox/FSTD/Manuscripts/FSTD-Response/Figures/Fig-3/Fig-3.pdf')
saveas(gcf,'~/Dropbox/FSTD/Manuscripts/FSTD-Response/Figures/Fig-3/Fig-3.pdf')
