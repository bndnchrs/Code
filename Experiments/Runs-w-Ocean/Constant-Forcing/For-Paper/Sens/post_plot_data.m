...
inds = [1 6 6*7 6*60];
inds(inds > size(DIAG.FSTD.psi,3)) = [];
nplots = length(inds);
...
div = size(lightplots,1)/max(nplots,3);
...
for ind = 1:3
    ...
    save_plaw{OPTS.run_number,ind,1} = plaw; 
    save_plaw{OPTS.run_number,ind,1} = horvat_alpha; 
