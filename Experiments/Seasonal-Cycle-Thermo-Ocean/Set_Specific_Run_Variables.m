...
THERMO.fixQ = 0; % Fix the heat flux and don't do the ocean modelling. 
...
if THERMO.fixQ
    ...
    % 400 heating from solar
    % 200 cooling from longwave.
    EXFORC.QSW = 400*cos(2*pi*OPTS.time/OPTS.year); 
    % Solar forcing may not be negative
    EXFORC.QSW(EXFORC.QSW < 0) = 0; 
    % Longwave forcing is constant 
    EXFORC.QLW = 250 + zeros(1,OPTS.nt);
