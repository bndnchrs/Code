function [THERMO] = setmner_1D_thermo_petty(FSTD,THERMO,OCEAN,EXFORC)
...
% category
% THERMO.F_s is the conductive flux per thickness category (positive = melting)
...
THERMO.Q_vert = zeros(1,length(h)); 
THERMO.F_s = THERMO.Q_vert; 
...
for i = 1:length(h)
    ...
    if OCEAN.DO && OCEAN.dopetty
        ...
        surf_HF = @(T_i) ...
            ...
            cond_HF(T_i); % Conductive heat flux out of ice surface
    else
 ...
 %%  Now fix if the temperature would allow for melting
    if THERMO.T_ice(i) > 0 % melting point of fresh sea ice
        ...
        THERMO.F_s(i) = cond_HF(THERMO.T_ice(i)); % < 0 almost always.
        ...
        % THERMO.F_s is the conductive heat flux 
        THERMO.F_s(i) = cond_HF(THERMO.T_ice(i));
...
THERMO.dhdt_base = -(THERMO.Q_bas/qbase + THERMO.F_s / qbase); 
